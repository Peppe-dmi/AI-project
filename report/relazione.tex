...
\usepackage{color}    % Per definire colori (es. \definecolor)
\usepackage{fancyhdr} % Per header e footer personalizzati
\usepackage{graphicx} % Per l'inclusione di immagini
\usepackage[british]{babel} % Gestione della lingua (opzionale: in un documento in italiano si potrebbe usare [italian])
\usepackage{hyperref} % Per i collegamenti ipertestuali (es. \href)
...
\usepackage{algorithmicx}   % Per scrivere pseudocodice
\usepackage{algpseudocode}  % Per l'ambiente pseudocodice
\urlstyle{same}             % Per uniformare lo stile degli URL
\usepackage[utf8]{inputenc}  % Codifica degli input
\usepackage[T1]{fontenc}     % Codifica dei font
\usepackage{imakeidx}        % Per creare l'indice
\makeindex[columns=1, title=Indice]
\usepackage{enumitem}        % Per personalizzare gli elenchi puntati e numerati
